...
namespace encryptor {
    ...
    void DpapiEncryptor::encrypt(DATA_BLOB* data, DATA_BLOB* entropy, DATA_BLOB* encryptedData){
        ...
        if(!CryptProtectData(
            ...
            encryptedData)){
				...
				throw std::system_error(errCode, std::system_category(), "Encryption error using CryptProtectData.\n");
        }
    }

    void DpapiEncryptor::decrypt(DATA_BLOB* encryptedData, DATA_BLOB* entropy, DATA_BLOB* data){
        ...
        if (!CryptUnprotectData(
            ...
            data)){
				auto errCode = GetLastError();
                throw std::system_error(errCode, std::system_category(), "Decryption error using CryptUnprotectData\n");
        }
    }
